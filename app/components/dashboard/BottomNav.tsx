...
export default function BottomNav({ activeTab, onTabChange }: Readonly<BottomNavProps>) {
        ...
        <nav className="md:hidden fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 pb-8 pt-4 bg-background/80 backdrop-blur-xl border-t border-outline-variant shadow-2xl">
            {items.map(({ icon: Icon, label }) => (
                <button
                    ...
                    className={`flex flex-col items-center justify-center rounded-2xl w-12 h-12 transition-all duration-200 cursor-pointer ${activeTab === label
